...
public class RegulationTexts {
   ...
   // @Bookmarkable
    ...
    public List<RegulationText> notYetFinalized() {
        final List<RegulationText> items = notYetFinalizedNoUi();
        if(items.isEmpty()) {
    ...
    @Programmatic
    public List<RegulationText> notYetFinalizedNoUi() {
    ...
    public List<RegulationText> isFinalized() {
        final List<RegulationText> items = isFinalizedNoUi();
        if(items.isEmpty()) {
    ...
    @Programmatic
    public List<RegulationText> isFinalizedNoUi() {
        return container.allMatches(
            new QueryDefault<RegulationText>(RegulationText.class,
    ...
    public RegulationText newRegulationText(
  ...
  //  public String default0NewRegulationText() {
   ...
   // }
    ...
    public List<RegulationText> allRegulations() {
        final List<RegulationText> items = container.allMatches(
                new QueryDefault<RegulationText>(RegulationText.class,
                        "findByOwnedBy", 
                        "ownedBy", currentUserName()));
        if(items.isEmpty()) {
   ...
   //         final boolean finalized,
    ...
    {
        final RegulationText regulationText = container.newTransientInstance(RegulationText.class);
        regulationText.setPlainRegulationText(plainRegulationText);
        regulationText.setRegulationTitle(regulationTitle);
        //regulationText.setAmendmentDate(amendmentDate);
        //regulationText.setFinalized(finalized);
        regulationText.setAmendmentDate(clockService.now());
        regulationText.setFinalized(false);
        regulationText.setOwnedBy(userName);
//        Object regId=JDOHelper.getObjectId(regulationText);
        regulationText.setRegulationTitle(regulationTitle);
        container.persist(regulationText);
        container.flush();
        regulationText.setRegulationTitle(regulationText.getIdString()+": "+ regulationTitle);
